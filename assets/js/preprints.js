...
// Load and display preprints from BibTeX
async function loadPublications() {
  try {
    const response = await fetch('preprints.bib');
    if (!response.ok) throw new Error('Failed to load preprints.bib');
    ...
    const projectList = document.querySelector('.project-list');
    if (projectList) {
      projectList.innerHTML = '<div>Error loading preprints. Please check console.</div>';
    }
  }
}
